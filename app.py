...
kb = DBpedia(one_hop_bloom_file="./data/blooms/spo1.bloom")
...
def query():
	...
	postprocess(generated_queries, count_query, ask_query)
...
def generate_query(question, question_type, entities, relations, ask_query=False, count_query=False):
	...
	graph = Graph(kb)	
	graph.find_minimal_subgraph(entities, relations, double_relation=double_relation, ask_query=ask_query, sort_query=sort_query, h1_threshold=h1_threshold)

	query_builder = QueryBuilder()
	valid_walks = query_builder.to_where_statement(graph, answer_parser.parse_queryresult, ask_query=ask_query, count_query=count_query, sort_query=sort_query)
	...
	if question_type == 0 and len(relations) == 1:
		...
		query_builder = QueryBuilder()
		graph.find_minimal_subgraph(entities, relations, double_relation=double_relation, ask_query=ask_query, sort_query=sort_query, h1_threshold=h1_threshold)
		valid_walks_new = query_builder.to_where_statement(graph, answer_parser.parse_queryresult, ask_query=ask_query, count_query=count_query, sort_query=sort_query)
...
def postprocess(generated_queries, count_query, ask_query):
	...
	for s in generated_queries:
		...
		for idx in range(len(sorted_queries)):
			...
			print(output_where)
			print("WHERE")
			print(kb.sparql_query(where["where"]))
...
def rank(args, question, generated_queries):
	...
	if 2 > 1:
		...
		vocab = Vocab(filename=dataset_vocab_file, data=[Constants.PAD_WORD, Constants.UNK_WORD, Constants.BOS_WORD, Constants.EOS_WORD])
		...
		# Prepare the dataset
		json_data = [{"id": "test", "question": question, "generated_queries": [{"query": " .".join(query["where"]), "correct": False} for query in generated_queries]}]
